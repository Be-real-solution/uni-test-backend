...
import { v4 } from 'uuid'
import { appConfig as config } from 'configs/app.configs'
...
export async function deleteFile(file_name: string) {
	try {
		...
		throw new HttpException('Error deleting file', 400)
	}
}
...
export async function createFile(file: any): Promise<string> {
	try {
		...
		const file_path = resolve(__dirname, '..', '..', '..', '..', config.path_for_file_upload)
		if (!existsSync(file_path)) {
